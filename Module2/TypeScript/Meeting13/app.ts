...
const hypotenuse = Number(prompt("Enter the length of the hypotenuse:"));
const sideA = Number(prompt("Enter the length of side A:"));
function calculateSide(hypotenuse: number, side: number): number {
    return Math.sqrt(hypotenuse * hypotenuse - side * side);
}
const sideB = calculateSide(hypotenuse, sideA);
